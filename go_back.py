...
class QRRTStar(Node):
    def __init__(self):
        ...
        self.robot_trajectory_publisher = self.create_publisher(Path, '/robot_reset_trajectory', 10)
        ...
        # 定时器用于周期性调用回调函数
        # self.create_timer(0.1, self.publish_robot_trajectory)
    ...
    def start_position_callback(self, msg:PoseStamped):
        self.get_logger().info('record restore position')
        self.goal=msg.pose
